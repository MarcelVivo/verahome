...
begin
  ...
  where id = p_invoice_id and status = 'offen' and archived_at is null
...
create policy invoices_update on public.invoices
  for update using (public.is_admin() or (archived_at is null and issuer_profile_id = auth.uid() and public.is_approved()))
  with check (public.is_admin() or (archived_at is null and issuer_profile_id = auth.uid() and public.is_approved()));
...
create policy invoice_line_items_select on public.invoice_line_items
  for select using (
    exists (select 1 from public.invoices i where i.id = invoice_line_items.invoice_id
      and (public.is_admin() or (i.archived_at is null and (i.issuer_profile_id = auth.uid() or i.recipient_profile_id = auth.uid()))))
  );
...
-- BEGIN DOCUMENT PRIVACY (source: document-privacy.sql)
