...
Der lokale SQL-Stand enthält heute sowohl explizite Dokumentfreigaben als auch automatisch aus Objekt-/Einheitsbeziehungen abgeleitete Leserechte (`supabase/document-vault-scoped-access.sql`). Sichtbarkeitsklassen und zeitliche Gültigkeit müssen deshalb vor einem Umbau der Ablage durchgängig definiert werden. Nur ein neues Auswahlfeld „privat“ reicht nicht: Tabellen-, Datei- und signierte Downloadzugriffe müssen dieselbe Entscheidung durchsetzen. Welche SQL-Version live aktiv ist, wird vor einer Migration abgeglichen.
